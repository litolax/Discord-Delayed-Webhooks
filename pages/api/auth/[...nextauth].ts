...
async function refreshAccessToken(token: any) {
    try {
        const url =
            process.env.HUB_PRIVATE_URL + '/auth/token';
        ...
        return {
            ...
            accessTokenExpires: new Date().getTime() / 1000 + refreshedTokens.expires_in,
...
export default NextAuth({
    ...
    callbacks: {
        ...
        async signIn(props) {
            ...
            if (!profile.SpecialPermissions || !profile.SpecialPermissions.includes("discordManager"))
                return "/unauthorized";
            return true;
        }
    }
